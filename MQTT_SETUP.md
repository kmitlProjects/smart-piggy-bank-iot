...
ส่วนนี้ใช้ยืนยันบทบาทของ MQTT ในระบบเวอร์ชันปัจจุบัน

- frontend ไม่ได้ subscribe broker โดยตรง
- frontend อ่านข้อมูลผ่าน backend APIs เท่านั้น
- backend เป็นตัวรับ `piggybank/data`, แปลงข้อมูล, บันทึกลง SQLite, และเปิด API ให้หน้าเว็บ
- command สำคัญ เช่น `unlock`, `reset`, `rfid_enroll_mode`, และ `dashboard refresh interval` ถูกส่งจาก backend ไป ESP32 ผ่าน `piggybank/command`
- RFID authorization ไม่ได้ตัดสินผ่าน MQTT แต่ ESP32 จะเรียก backend `/api/access/check` โดยตรงผ่าน HTTP
- MQTT ทำหน้าที่เป็น `device-backend messaging layer` ไม่ใช่ frontend realtime transport

ตัวอย่างคำอธิบายที่ใช้ในรายงานได้:

> In the final system, MQTT is used strictly as the communication layer between the ESP32 device and the backend service. The frontend does not subscribe to MQTT directly; instead, it consumes processed data from the backend through REST APIs. This design centralizes authorization, logging, and data transformation in the backend.
