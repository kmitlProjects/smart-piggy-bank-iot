...
ตัวอย่าง:
```bash
./tools/sync_up.sh auto YOUR_PASSWORD 10.164.223.245
```
...
├─ /api/status
├─ /api/device
├─ /api/coins/*           (summary, history)
├─ /api/access/*          (authorization, access logs)
├─ /api/statistics        (timeseries-derived analytics)
├─ /api/transactions      (deposits + access + security + system logs)
├─ /api/rfid/*            (card management, enroll mode)
├─ /api/unlock
├─ /api/reset
├─ MQTT subscriber        (piggybank/data -> SQLite)
└─ SQLite database        (coin events, access logs, RFID cards, connectivity,
                           latest status, activity events, device settings/runtime)
             │
             │ MQTT
             v
MQTT Broker (Mosquitto)
├─ Topic: piggybank/data
└─ Topic: piggybank/command
             │
             │ MQTT pub/sub
             v
ESP32 (MicroPython)
├─ RFID reader -> auth.py -> backend /api/access/check
├─ Coin counter with local persistence
├─ Ultrasonic sensor
├─ Lock relay (unlock only if authorized)
├─ OLED status display
├─ Periodic heartbeat publish (default 5s, configurable)
└─ Optional local status page via webserver.py
```

### Validated System Overview

ภาพรวมด้านบนเป็นโครงสร้างที่ตรวจแล้วว่าตรงกับโค้ดปัจจุบันของโปรเจกต์ ไม่ใช่โครงสร้างจากแผนเดิม โดยมีประเด็นที่ควรรู้ดังนี้

- frontend เวอร์ชันปัจจุบันมี 4 หน้า คือ `Dashboard`, `Statistics`, `Transactions`, และ `Settings`
- frontend ไม่ได้ subscribe MQTT โดยตรง แต่เรียกข้อมูลผ่าน backend APIs
- backend เป็นจุดศูนย์กลางของ logging, statistics, RFID authorization, และ device state
- transaction feed ปัจจุบันไม่ได้มีเฉพาะ access logs แต่รวม `deposit`, `access`, `security`, และ `system activity`
- heartbeat ของ ESP32 มีค่า default ที่ `5 วินาที` แต่สามารถปรับได้จากระบบ settings
- ESP32 มี local web status page อยู่จริง แต่เป็นส่วนเสริม ไม่ใช่เส้นทางหลักของระบบ
